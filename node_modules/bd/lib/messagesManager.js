  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var WorkspacesMessages = declare( [ HotStore, MultiHomePermsMixin ],  {
      ...
      queryConditions: {
        name: 'and',
        args: [
          { name: 'eq', args: [ 'incoming', '#incoming#' ] },
          { name: 'eq', args: [ 'type', '#type#' ] },
          { name: 'eq', args: [ 'fromContactId', '#fromContactId#' ] },
          { name: 'eq', args: [ 'messageId', '#messageId#' ] },
          { name: 'eq', args: [ 'workspacesMessagesTo.id', '#toContactId#' ] },
          {
            name: 'or', ifDefined: 'contactId', args: [
              { name: 'eq', args: [ 'workspacesMessagesTo.contactId', '#contactId#' ] },
              { name: 'eq', args: [ 'fromContactId', '#contactId#' ] },
            ]
          }
        ]
        ...
        var doc = this._co( doc );
				...
				hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
					...
					hotCoreSharedCode.getAllSharedFunctions( function( err, sharedFunctions ){
						...
						//storeRegistry.workspacesMessagesTo.apiGetQuery( { conditions: { workspaceId: doc.workspaceId, messageId: doc.messageId } }, function( err, tos ){
      ...
      _selectFromContactId: function( request, done ){
	        ...
	        if( request.remote ){
		    	  ...
		    	  debug("It's a remote request. Runnng a query on workspacesContacts looking for the request's userId. Query:", { ranges: { limit: 1 }, conditions: { userId: request._req.session.userId } } );

	          storeRegistry.workspacesContacts.apiGetQuery( { ranges: { limit: 1 }, conditions: { userId: request._req.session.userId } }, function( err, docs ){
	            if( err ) return done( err );
...
hotplate.hotEvents.on( 'augmentMessageDeliveryLog', 'bd', hotplate.cachable( function( o, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    storeRegistry.workspacesMessages.apiGetQuery( { conditions: { messageId: o.messageId } }, function( err, workspaceMessage){
      if( err ) return done( err );
    ...
    switch( transport ){
      ...
      case 'sms-twilio':
        ...
        s.apiGetQuery( {}, function( err, smsGatewayConfig ){
          ...
          storeRegistry.phoneNumbers.apiGetQuery( { conditions: { provider: provider } }, function( err, phoneNumbers ){
            if( err ) return done( err );
...
hotplate.hotEvents.on( 'transportConfig', function( messageTo, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    storeRegistry.workspacesMessages.apiGetQuery( { sort: { added: 1 }, conditions: { messageId: messageId } }, function( err, workspacesMessages){
      if( err ) return done( err );
...
hotplate.hotEvents.on( 'transportMessageFetched', 'bd', function( transport, config, messageRecord, messageToRecord, cb ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    function workoutMessagesToContactId( cb ){
      ...
      storeRegistry.workspacesContacts.apiGetQuery( { ranges: { limit: 1 }, conditions: { workspaceId: config.workspaceId, id: id } }, function( err, docs ){
    ...
    function workoutMessagesFromContactId( cb ){
      ...
      storeRegistry.workspacesContactsEmailsBase.apiGetQuery( { ranges: { limit: 1 }, conditions: { workspaceId: config.workspaceId, email: email } }, function( err, docs ){
    ...
    if( messageRecord.type == 'email' ){
          ...
          storeRegistry.workspacesMessages.apiPost( wm, function( err, workspaceMessage ){
            ...
            storeRegistry.workspacesMessagesTo.apiPost( wmt, function( err ){
              if( err ) {
							  ...
							  storeRegistry.workspacesMessages.dbLayer.delete( { name: 'eq', args: [ 'id', workspaceMessage.id ] }, { multi: true }, function( err ){
      ...
      storeRegistry.phoneNumbers.apiGetQuery({ ranges: { limit: 1 }, conditions: { fullNumber: messageToRecord.to } }, function( err, docs ){
        if( err ) return cb( err );
        ...
        storeRegistry.workspacesMessagesToLookup.apiGetQuery( { conditions: filter, sort: { added: 1 } }, function( err, docs ){
        ...
        function nextStep( lookups ){
          ...
          async.eachSeries(
            ...
            function( lookupEntry, cb){
              ...
              storeRegistry.workspacesContactsPhoneNumbers.apiGetQuery({ conditions: { fullNumber: messageRecord.from, workspaceId: lookupEntry.workspaceId } }, function( err, docs ){
