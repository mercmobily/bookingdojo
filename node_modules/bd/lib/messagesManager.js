...
, hotCoreJsonRestStores = require('hotplate/node_modules/hotCoreJsonRestStores')
, hotCoreMultiHome = require('hotplate/node_modules/hotCoreMultiHome')
, hotCoreSharedCode = require('hotplate/node_modules/hotCoreSharedCode')
, hotCoreStoreRegistry = require( 'hotplate/node_modules/hotCoreStoreRegistry' )
, hotCoreServerLogger = require( 'hotplate/node_modules/hotCoreServerLogger' )
, logger = hotCoreServerLogger
, hotCoreTransport = require( 'hotplate/node_modules/hotCoreTransport')
;

var logLine = hotCoreTransport.transportLayerFunctions.logLine;

hotplate.hotEvents.onCollect( 'stores', 'bd', hotplate.cacheable( function( done ){
  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var WorkspacesMessages = declare( [ HotStore, MultiHomePermsMixin ],  {
      ...
      schema: new HotSchema({
        ...
        added          : { type: 'date', required: false, searchable: true,  protected: true, default: function() { return new Date() } },
        ...
        //toAsString          : { type: 'string', searchable: false, protected: true }, // Computed

        // DoNotSave fields (their value will be stored in hotCoreTransport's `messages` store)

        // These only apply in POST methods
        to: {
          type: 'string',
          required: false,
          doNotSave: true,
        ...
        type: {
          ...
          validator: function( o, v, fieldName ){
            if( v && v !== 'email' && v !== 'sms' ) return "Type must be 'sms' or 'email' ";
          }
        },
        subject: {
          type: 'string',
      ...
      queryConditions: {
        type: 'and',
        args: [
          { type: 'eq', args: [ 'incoming', '#incoming#' ] },
          { type: 'eq', args: [ 'type', '#type#' ] },
          { type: 'eq', args: [ 'fromContactId', '#fromContactId#' ] },
          { type: 'eq', args: [ 'messageId', '#messageId#' ] },
          { type: 'eq', args: [ 'workspacesMessagesTo.id', '#toContactId#' ] },
          {
            type: 'or', ifDefined: 'contactId', args: [
              { type: 'eq', args: [ 'workspacesMessagesTo.contactId', '#contactId#' ] },
              { type: 'eq', args: [ 'fromContactId', '#contactId#' ] },
            ]
          }
        ]
      ...
      nested: [
        ...
        /*
        {
          store: 'workspacesMessagesAttachments',
          join: { 'workspaceMessageId': 'id' },
          type: 'multiple'
        },
        */
        ...
        {
          type: 'lookup',
          localField: 'fromContactId',
          store: 'workspacesContacts',
          //layerField: 'id'
        },
        ...
        var doc = this._co( doc );
				...
				hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
					...
					hotCoreSharedCode.getAllSharedFunctions( function( err, sharedFunctions ){
						...
						//storeRegistry.workspacesMessagesTo.dbLayer.selectByHash( { conditions: { workspaceId: doc.workspaceId, messageId: doc.messageId } }, function( err, tos ){

            /*
            storeRegistry.workspacesMessagesTo.dbLayer.selectHash( { conditions:
            storeRegistry.workspacesMessagesTo.dbLayer.select( { conditions:
              { name: 'and',
                args: [
                  { name: 'eq', args: [ 'workspaceId', doc.workspaceId ] },
                  { name: 'eq', args: [ 'messageId', doc.messageId ] }
                ]
              }
            }, { children: true }, function( err, tos ){
            */


            storeRegistry.workspacesMessagesTo.dbLayer.selectByHash( { conditions: { workspaceId: doc.workspaceId, messageId: doc.messageId } }, { children: true }, function( err, tos ){
							...
							async.eachSeries(
								...
								function( workspaceTo, cb ){
								  ...
								  var contactName, contactMeans, incoming, outgoing;

								  // DELETE ME: In new data, this should never happen
								  if( workspaceTo.contactId && typeof( workspaceTo._children.contactId) === 'undefined' ){
								  	result.push( "??");
								  	return cb( null );
								  }
								  if( workspaceTo.contactId && typeof( workspaceTo._children.contactId) === 'undefined' ){
								  ...
								  incoming = typeof( workspaceTo._children.workspaceMessageId ) !== 'undefined' &&  workspaceTo._children.workspaceMessageId.incoming;
								  outgoing = ! incoming;

									if( workspaceTo.contactId && workspaceTo._children.contactId ){
										contactName = sharedFunctions.renderContactName( workspaceTo._children.contactId );
									}
									if( workspaceTo.messageToId && workspaceTo._children.messageToId ){
										contactMeans = workspaceTo._children.messageToId.to;
									}
								    ...
								    if( contactName ){
										  result.push( contactName + " (" + contactMeans + ")");
										} else if( contactMeans ){
  								  	result.push( contactMeans );
										} else {
											result.push( " ?? ");
										}
									}
      ...
      // afterCheckPermissions() and  afterDbOperation(), which are executed one after the other
      ...
      _selectFromContactId: function( request, done ){
	        ...
	        if( request.remote ){
		    	  ...
		    	  debug("It's a remote request. Runnng a query on workspacesContacts looking for the request's userId. Query:", { ranges: { limit: 1 }, conditions: { userId: request.session.userId } } );

	          storeRegistry.workspacesContacts.dbLayer.selectByHash( { ranges: { limit: 1 }, conditions: { userId: request.session.userId } }, function( err, docs ){
	            if( err ) return done( err );
      ...
      afterCheckPermissions: function( request, method, done ){
        ...
        var self = this;
      	...
      	// Don't do ANYTHING if it's not adding a new element (post), for it's incoming
        ...
        // it's already in the hotTransport tables. So, there is no need to do any of this)
				...
				debug("In afterCheckPermissions, request.body is: ", request.body );
        ...
        hotCoreStoreRegistry.getAllStores( function( err, storeRegistry ){
          ...
          // ***********************
          // STEP 1: Work out fromContactId, from request (remote call) or passed parameter
          // ***********************
          ...
          self._selectFromContactId( request, function( err, fromContactId ){
            ...
            storeRegistry.messages.dbLayer.insert( o, function( err, res ){
              if( err ) return done( err );
      ...
      // So, there we go...
      // ******************************************************************
      afterDbOperation: function( request, method, done ){
        ...
        // it's already in the hotTransport tables. So, there is no need to do any of this)
				...
				debug("In afterDbOperation, request.body is: ", request.body );
        ...
        async.eachSeries(
          ...
          function( id, cb ){
            ...
            request.kept.workspacesContactsXXX.dbLayer.selectById( id, function( err, info ){
              if( err ){
              ...
              request.kept.storeRegistry.messagesTo.dbLayer.insert( o, function( err, messageTo ){
                ...
                var o = {
                  ...
                  workspaceMessageId: request.data.fullDoc.id,
                ...
                request.kept.storeRegistry.workspacesMessagesTo.dbLayer.insert( o, function( err, workspaceMessageTo ){
                  if( err ) return cb( err );
                  ...
                  var f = { conditions: { workspaceId: request.body.workspaceId, to: messageTo.to, type: request.kept.type } };
                  debug( "Matching incoming message in workspacesMessagesToLookup, filter: ", f );
                  request.kept.storeRegistry.workspacesMessagesToLookup.dbLayer.selectByHash( f, function( err, docs ){
                    ...
                    if( docs.length ){
                      ...
                      request.kept.storeRegistry.workspacesMessagesToLookup.dbLayer.updateById( lookup.id, { added: new Date() }, function( err ){
                        if( err ){
                          logger.log( {
                            system: true,
                            logLevel: 3,
                            message: "Could not update date field in entry in workspacesMessagesToLookup",
                            data: { },
                            error: err,
                            workspaceId: request.body.workspaceId
                          });
                    ...
                    // Entry not there: save a new one with all of the info
                      ...
                      request.kept.storeRegistry.workspacesMessagesToLookup.dbLayer.insert( lookup, function( err ){
                        if( err ){
                          logger.log( {
                            system: true,
                            logLevel: 3,
    ...
    var WorkspacesMessagesTo = declare( [ HotStore, MultiHomePermsMixin ],  {
      ...
      nested: [
        ...
        {
          type: 'lookup',
          localField: 'workspaceMessageId',
          store: 'workspacesMessages',
          //layerField: 'id'
          //join: { 'id': 'workspaceMessageId' },
        },
...
hotplate.hotEvents.onCollect( 'augmentMessageDeliveryLog', 'bd', function( o, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    storeRegistry.workspacesMessages.dbLayer.selectByHash( { conditions: { messageId: o.messageId } }, function( err, workspaceMessage){
      if( err ) return done( err );
...
hotplate.hotEvents.onCollect( 'allTransportConfig', 'bd', function( transport, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    switch( transport ){
      ...
      case 'email-default':
        ...
        storeRegistry.workspacesEmailConfig.dbLayer.selectByHash( {}, function( err, workspaceEmailConfigRecords ){
          if( err ) return done( err );
      ...
      case 'sms-twilio':
        ...
        s.dbLayer.selectByHash( {}, function( err, smsGatewayConfig ){
          ...
          storeRegistry.phoneNumbers.dbLayer.selectByHash( { conditions: { provider: provider } }, function( err, phoneNumbers ){
            if( err ) return done( err );
...
hotplate.hotEvents.onCollect( 'transportConfig', function( messageTo, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    storeRegistry.workspacesMessages.dbLayer.selectByHash( { sort: { added: 1 }, conditions: { messageId: messageId } }, function( err, workspacesMessages){
      if( err ) return done( err );

      var l = workspacesMessages.length;

      if( l === 0 ) done( new Error("Could not lookup workspacesMessage from messageId!"));
      if( l > 1 ) done( new Error("Found more than 1 workspacesMessage from one messageId!"));
      ...
      switch( type ){
        ...
        case 'email':
          ...
          storeRegistry.workspacesEmailConfig.dbLayer.selectById( workspaceId, function( err, workspaceEmailConfig ){
            if( err ) return done( err );
        ...
        case 'sms':
          ...
          storeRegistry.workspacesSmsConfig.dbLayer.selectById( workspaceId, function( err, workspaceSms ){
...
hotplate.hotEvents.onCollect( 'transportManipulateBeforeSend', function( transport, messageTo, config, done ){
...
hotplate.hotEvents.onCollect( 'transportManipulateMessageBeforeSave', function( transport, message, config, done ){
...
hotplate.hotEvents.onCollect( 'transportMessageFetched', 'bd', function( transport, config, messageRecord, messageToRecord, cb ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    ...
    function workoutMessagesToContactId( cb ){
      ...
      storeRegistry.workspacesContacts.dbLayer.selectByHash( { ranges: { limit: 1 }, conditions: { workspaceId: config.workspaceId, id: id } }, function( err, docs ){
    ...
    function workoutMessagesFromContactId( cb ){
      ...
      debug("Running a search. Filter:", { ranges: { limit: 1 }, conditions: { workspaceId: config.workspaceId, email: email } } );

      storeRegistry.workspacesContactsEmailsBase.dbLayer.selectByHash( { ranges: { limit: 1 }, conditions: { workspaceId: config.workspaceId, email: email } }, function( err, docs ){
    ...
    if( messageRecord.type == 'email' ){
      ...
      workoutMessagesToContactId( function( err, messagesToContactId ){
        ...
        workoutMessagesFromContactId( function( err, messagesFromContactId ){
          ...
          // At this point, I want to create a workspaceMessage entry with a "apiPut".
          // Trouble is, I want to create it _whole_, incuding the children.
          // For this, I will make the ID artificially, add an entry to workspaceMessagesTo
          // artificially, and then do an apiPut.
          // TODO: Change simpleDbLayer/JsonRestStores so that if body has _children,
          // and if attributes in _children match childen tables, then adds those
          // records first. Basically, allow adding of records with children in one call

          // Make up the ID for workspaceMessageId, used to create workspaceMessagesTo entries
          // before the workspaceMessage entry itself
          storeRegistry.workspacesMessages.schema.makeId( {}, function( err, workspaceMessageId ){
            if( err ) return cb( err );

            // Make up the messagesTo object. Note that contactId might be set
            // or not, depending on whether something was found
            var wmt = {
              workspaceMessageId: workspaceMessageId, // Generated
            ...
            storeRegistry.workspacesMessagesTo.dbLayer.insert( wmt, function( err ){
              if( err ) return cb( err );

              // Make up the object. Note that fromContactId might be set
              // or not, depending on whether something was found
              var wm = {
                id: workspaceMessageId, // Generated
                workspaceId: config.workspaceId,
                incoming: true,
              ...
              debug( "About to store via apiPut:" );
              debug( "Message:", wm );

              storeRegistry.workspacesMessages.apiPut( wm, function( err, workspaceMessage ){

                // If there was a problem with the apiPut, then delete the entry we just created
                // in workspacesMessagesTo since it would be an orphan
                if( err ) {
                  storeRegistry.workspacesMessagesTo.dbLayer.delete( { type: 'eq', args: [ 'workspaceMessageId', workspaceMessageId ] }, { multi: true }, function( err ){
        ...
        }); //workoutMessagesFromContactId
      }); //workoutMessagesToContactId

    } else if( messageRecord.type == 'sms' ){

      debug("It's an SMS!");

      storeRegistry.phoneNumbers.dbLayer.selectByHash( { ranges: { limit: 1 }, conditions: { fullNumber: messageToRecord.to } }, function( err, docs ){
        if( err ) return cb( err );
        ...
        storeRegistry.workspacesMessagesToLookup.dbLayer.selectByHash( { conditions: filter, sort: { added: 1 } }, function( err, docs ){
        ...
        function nextStep( lookups ){
          ...
          async.eachSeries(
            ...
            function( lookupEntry, cb){
              ...
              storeRegistry.workspacesContactsPhoneNumbers.dbLayer.selectByHash( { conditions: { fullNumber: messageRecord.from, workspaceId: lookupEntry.workspaceId } }, function( err, docs ){
                  ...
                  storeRegistry.workspacesMessagesTo.dbLayer.insert( wmt, function( err ){
                    ...
                    storeRegistry.workspacesMessages.apiPut( wm, function( err, workspaceMessage ){
                      if( err ){
                        ...
                        storeRegistry.workspacesMessagesTo.dbLayer.delete( { type: 'eq', args: [ 'workspaceMessageId', workspaceMessage.id ] }, { multi: true }, function( err ){
                          logLine( config, null, null, 3, "Error deleting entry from workspacesMessages (doing so because adding to workspacesMessagesTo failed!)", err );
                        });
