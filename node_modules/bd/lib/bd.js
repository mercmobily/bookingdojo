...
hotplate.hotEvents.on( 'run', 'bd', hotplate.cachable( function( done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry ){
    ...
    storeRegistry.workspacesUsersBase.dbLayer.on( 'insert', function( record ){

      storeRegistry.workspacesContacts.dbLayer.insert( { userId: record.userId, workspaceId: record.workspaceId }, function( err ){
        console.log( record );

        // TODO: LOG THE ERROR SOMEWHERE IF THERE IS A PROBLEM HERE
        if( err ){
  ...
  hotCoreJsonRestStores.get( function( err, s ){
        ...
        args: [

          {
            ...
            args: [
              ...
              { name: 'startsWith', args: [ 'workspacesContactsAddresses.street','#searchAll#' ] },
            ]
          },
          ...
          },
          {
            ...
            name: 'or',
            args: [
              { name: 'eq', args: [ 'workspacesContactsPhoneNumbers.fullNumber', '#searchFrom#' ] },
              { name: 'eq', args: [ 'workspacesContactsEmails.email', '#searchFrom#' ] },
            ]
          },
        ]
      },
    ...
    stores.workspacesContacts = new WorkspacesContacts();
      ...
      queryConditions: {
        ...
        args: [
          ...
          { name: 'and', args: [
            { ifDefined: 'searchAll', name: 'startsWith', args: [ 'workspacesContactsEmails.email', 'm' ] },
            { name: 'startsWith', args: [ 'workspacesContactsEmails.email', 'n' ] },
            { name: 'startsWith', args: [ 'workspacesContactsEmails.email', 'o' ] },
            { name: 'startsWith', args: [ 'workspacesContactsEmails.email', 'p' ] },
            { ifDefined: 'searchAll', name: 'and', args: [ 
              { name: 'startsWith', args: [ 'workspacesContactsEmails.email', 'IFDEFINED' ] },
              { name: 'startsWith', args: [ 'workspacesContactsEmails.email', 'IFDEFINED' ] },
            ]}
    ...
    var UpdateFirstParentMixin = declare( {
      ...
      _setMain: function( request, cb ){
        ...
        self.dbLayer.selectByHash( { ranges: { limit: 1 }, conditions: { contactId: request.params.contactId } }, function( err, docs ){
          ...
          stores.workspacesContacts.dbLayer.update( { name: 'eq', args: [ 'id', request.params.contactId ] }, updateObject, function( err, n ){
            if( err ) return cb( err );
    ...
    var WorkspacesContactsEmailsSearch = declare( [ WorkspacesContactsEmailsBase ],  {
      ...
      queryConditions:{
        ...
        args: [
          {
            name: 'eq', args: [ 'contactId.isStarred', '#isStarred#' ],
          },

          {
            ...
            ifDefined: 'searchAll',
            args: [
              { name: 'startsWith', args: [ 'email', '#searchAll#' ] },
              { name: 'startsWith', args: [ 'contactId.firstName', '#searchAll#' ] },
              { name: 'startsWith', args: [ 'contactId.lastName', '#searchAll#' ] },
            ]
          }
        ]
      },
   ...
   var WorkspacesContactsPhoneNumbersSearch = declare( [ WorkspacesContactsPhoneNumbersBase ],  {
    ...
    var PhoneNumbers = declare( [ HotStore ], {
      ...
      queryConditions: {
        name: 'and',
        args: [
          { name: 'contains', args: [ 'fullNumber', '#fullNumber#'] },
          { name: 'eq', args: [ 'type', '#type#'] },
          { name: 'eq', args: [ 'provider', '#provider#'] },
          { name: 'eq', args: [ 'workspaceId', '#workspaceId#'] },

          {
            name: 'or',
            args: [
              { name: 'eq', args: [ 'workspaceId', '#workspaceIdFilter#'] },
              { name: 'eq', args: [ 'type', 'shared' ] },
            ]
          }
        ]
      },
  ...
  switch(pageName){
    ...
    case 'hotDojoAppContainer/container':
      ...
      hotCoreStoreRegistry.getAllStores( function( err, storeRegistry ){
        ...
        storeRegistry.workspacesContacts.dbLayer.selectByHash( { userId: req.session.userId }, function( err, record ){
          if( err ) return done( err );
