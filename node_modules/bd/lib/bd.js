...
/*
hooks.enrichDojoConf = function( done, conf ){

  var p = path.join( hotplate.get( 'staticUrlPath' ), 'bd', 'dgrid' );
...
hooks.run = function( done ){
  ...
  // *****************************************************
  // Hotplate-specific advancements to Schema and Store
  // (To be moved)
  // *****************************************************

  Schema.prototype.sharedValidatorTypeParam = function( p ){
  ...
  var Categories = declare( HotStore, {
    ...
    // COMMON
    schema: new Schema({
      _id     : { type: 'id', isRequired: true, searchable: true, sortable: true },
  ...
  Categories.make.All( app, '/call/:workspaceIdCall/Categories/', ':catId', Categories );
  ...
  var UserConfig = declare( HotStore, {
    ...
    schema: new Schema({
      ...
      email   : { type: 'string', notEmpty: true, sharedValidator: 'email', trim: 70, min: 10 },
...
hooks.stores = function( done ){
    ...
    categories           : { target: '/call/:workspaceIdCall/Categories/',      idProperty: '_id', sortParam: 'sortBy', },
  });
}
