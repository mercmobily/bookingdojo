...
hotplate.hotEvents.onCollect( 'run', 'bd', function( done ){
...
hotplate.hotEvents.onCollect( 'stores', 'bd', hotplate.cacheable( function( done ){
  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var WorkspacesContacts = declare( [ HotStore, MultiHomePermsMixin ],  {
            ...
            args: [
              {
                type: 'or',
                args: [
                  { type: 'startsWith', args: [ 'firstName', '#searchAllEach#' ] },
                  { type: 'startsWith', args: [ 'lastName', '#searchAllEach#' ] },
                  { type: 'startsWith', args: [ 'workspacesContactsEmails.email', '#searchAllEach#' ] },
                  { type: 'startsWith', args: [ 'workspacesContactsAddresses.street','#searchAllEach#' ] },
                ]
              }
      ...
      nested: [

        {
          ...
          store: 'workspacesContactsEmails',
        },

        {
          type: 'lookup',
          localField: 'mainAddressId',
          store: 'workspacesContactsAddresses',
        },

        {
          type: 'lookup',
          localField: 'mainPhoneNumberId',
          store: 'workspacesContactsPhoneNumbers',
        },
      ...
      //alsoNotify: [ 'workspacesContacts2' ],
    ...
    var UpdateFirstParentMixin = declare( {
      ...
      // Redefine this store's dbLayer's reposition() method to set
      // the workspacesContacts's mainEmaidId field
      init: function f(){
        this.inherited( f, arguments );

        var mainField = this.firstParentStoreField;

        // TODO: This is a good use case for the documentation, where the
        // dblayer's methods need to be tweaked a little.
        // Also explain how I just redefined `reposition`, but I could have passed
        // an enhanged dbLayer as well
        this.dbLayer.reposition = function f( record, where, beforeId, cb ){

          var self = this;

          this.inheritedAsync( f, arguments, function( err ){

            self.selectByHash( { ranges: { limit: 1 }, conditions: { contactId: record.contactId } }, function( err, docs ){
              ...
              var updateObject = {};
              updateObject[ mainField ] = doc.id;

              // Update the corresonding contact
              stores.workspacesContacts.dbLayer.update( { type: 'eq', args: [ 'id', record.contactId ] }, updateObject, function( err, n ){
                if( err ) return cb( err );
      ...
      afterEverything: function f( request, method, done ){
        ...
        self.inheritedAsync( f, arguments, function( err ){
          ...
          // Only dealing with methods that manipulate data
          if( method !== 'putNew' && method !== 'putExisting' && method !== 'post') return done( null );

          var doc = request.data.preparedDoc;

          // TODO: This is a good use-case. Add this to the documentation (especially undefining beforeId)
          stores.workspacesContacts.dbLayer.selectById( doc.contactId, function( err, contact ){
            if( err ) return done( err );
            ...
            stores.workspacesContacts.broadcastStoreChanges( request, 'storeRecordUpdate', contact.id, contact, { keepForm: true, beforeId: undefined }, done );
          });
        });
      }
    ...
    var WorkspacesContactsAddresses = declare( [ HotStore, MultiHomePermsMixin, UpdateFirstParentMixin ],  {
      ...
      schema: new HotSchema({
        isMain            : { type: 'boolean', protected: true },
   ...
   var WorkspacesContactsPhoneNumbersSearch = declare( [ WorkspacesContactsPhoneNumbersBase ],  {
    ...
    // TODO: Make sure that non-god users MUST filter by workspaceIdFilter, and CANNOT filter by anything else
...
hotplate.hotEvents.onCollect( 'dojoModulesPerPage', 'bd', function( req, pageName, done ){
  ...
  switch( pageName ){
    ...
    case 'hotClientDojo/container':
      done( null, [ 'storeEnhancer', 'bdMain' ] );
    break;
  ...
  switch(pageName){
    ...
    case 'hotClientDojo/container':
      ...
      hotCoreStoreRegistry.getAllStores( function( err, storeRegistry ){
        ...
        storeRegistry.workspacesContacts.dbLayer.selectByHash( { conditions: { workspaceId: req.params.workspaceId, userId: req.session.userId } }, function( err, record ){
          if( err ) return done( err );
          if( !record[ 0 ] ) return done( new Error("Could not find contact associated to user") );
...
hotplate.hotEvents.onCollect( 'sharedFunctions', 'bd', hotplate.cacheable( function( done ) {
