...
hotplate.hotEvents.on( 'run', 'bd', hotplate.cachable( function( done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry ){
    ...
    storeRegistry.workspacesUsersBase.redefineMethod( 'afterEverything', function( request, method, p, cb ){
      this.inheritedAsync( storeRegistry.workspacesUsersBase.afterEverything, arguments, function( err ){
        if( err ) return done( err );
    ...
    storeRegistry.workspacesUsers.redefineMethod( 'afterEverything', function( request, method, p, cb ){
      this.inheritedAsync( storeRegistry.workspacesUsers.afterEverything, arguments, function( err ){
        if( err ) return done( err );
    ...
    storeRegistry.usersWorkspaces.redefineMethod( 'afterEverything', function( request, method, p, cb ){
      this.inheritedAsync( storeRegistry.usersWorkspaces.afterEverything, arguments, function( err ){
        if( err ) return done( err );
  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var WorkspacesContacts = declare( [ HotStore, MultiHomePermsMixin ],  {
      ...
      onlineSearchSchema: new HotSchema({
        ...
        searchFrom      : { type: 'string', trim: 70, searchable: true, searchOptions: [
          { field: 'workspacesContactsPhoneNumbers.fullNumber',  type: 'is', condition: 'or' },
          { field: 'workspacesContactsEmails.email',  type: 'is', condition: 'or' },
        ] },
    ...
    var UpdateFirstParentMixin = declare( null,  {
      ...
      afterEverything: function afterEverything( request, method, p, done ){
        ...
        var self = this;
        self.inheritedAsync( afterEverything, arguments, function( err ){
          if( err ) done( err );
   ...
   var WorkspacesContactsPhoneNumbersSearch = declare( [ WorkspacesContactsPhoneNumbersBase ],  {
    ...
    var PhoneNumbers = declare( [ HotStore ], {
      ...
      schema: new HotSchema({
        ...
        workspaceId      : { type: 'id', required: false, notEmpty: true, searchable: true, searchOptions: { type: 'is' }, validator: function( o, v, fieldName ){  if( o.type == 'private' && ! v ) return "You need to specify the workspace ID for private phone numbers" } },
        workspaceIdFilter: { type: 'id', searchable: true, doNotSave: true, searchOptions: [ { field: 'ownedByWorkspaceId', type: 'is', condition: 'or', }, { field: 'type', type: 'is', value: 'shared', condition: 'or' } ] }
      }),
