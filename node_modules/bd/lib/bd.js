  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var WorkspacesUsersInfo = declare( [ HotStore, MultiHomePermsMixin, PrivateUserDataMixin ], {
      ...
      schema: new HotSchema({
        ...
        setting3          : { type: 'id',  required: false },
      }),
        ...
        self.apiGetQuery( { ranges: { limit: 1 }, filters: { contactId: request.params.contactId } }, function( err, docs ){
          ...
          stores.workspacesContacts.dbLayer.update( { conditions: { and: [  { field: 'id', type: 'eq', value: request.params.contactId } ] } }, updateObject, function( err ){
            ...
            stores.workspacesContacts.apiGet( request.params.contactId, function( err, contact ){
              ...
              // NOTE: I pass `beforeId` so that `broadcastStoreChanges` doesn't take it from the
              // HTTP headers, as the original call here is on the workspacesContactsEmails store,
              // and the putBeforewould be wrong
              // FIXME: Find a better solution to suppress HTTP header defaults in broadcastStoreChanges
              ...
              stores.workspacesContacts.broadcastStoreChanges( request, 'storeRecordUpdate', contact.id, contact, { tabId: null, keepForm: true, putBefore: null }, cb );
            });
          });
    ...
    var WorkspacesContactsEmails = declare( [ HotStore, MultiHomePermsMixin, UpdateFirstParentMixin ],  {
      ...
      //alsoNotify: [ 'workspacesContactsEmails2'],
    ...
    var WorkspacesContactsEmails2 = declare( [ WorkspacesContactsEmails ],  {

      storeName:  'workspacesContactsEmails2',
      collectionName: 'workspacesContactsEmails',

      publicURL: '/workspaces/:workspaceId/contacts/:contactId/emails2TWO/:id',

      alsoNotify: [ 'workspacesContactsEmails']
    });
    stores.workspacesContactsEmails2 = new WorkspacesContactsEmails2();
