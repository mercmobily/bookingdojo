   ...
   var WorkspacesContactsPhoneNumbersSearch = declare( [ WorkspacesContactsPhoneNumbersBase ],  {
    ...
    var WorkspacesMessages = declare( [ HotStore, MultiHomePermsMixin ],  {
      ...
      schema: new HotSchema({
        ...
        messageId         : { type: 'id', required: false, protected: true, notEmpty: true },
        ...
        subject           : { type: 'string', searchable: true, doNotSave: true, required: false, notEmpty: false, trim: 1024 },
        ...
        hotCoreStoreRegistry.getAllStores( function( err, storeRegistry ){
          ...
          self._selectFromContactId( request, function( err, fromContactId ){
            ...
            storeRegistry.messages.apiPost( { 
              ...
              body: body.body,
              from: 'fixme@fixme.com' // FIXME: From workspace config
            }, function( err, res ){
              if( err ) return done( err );
    ...
    var WorkspacesMessagesTo = declare( [ HotStore, MultiHomePermsMixin ],  {
      ...
      schema: new HotSchema({
        contactId          : { type: 'id', required: true, searchable: true },

        messageToId       : { type: 'id', required: true, searchable: true },        
    ...
    // Root-level store
    var TwilioPhoneNumbers = declare( [ HotStore ], {

      schema: new HotSchema({
        number         : { type: 'number', notEmpty: true, trim: 50, searchable: true },
      }),

      onlineSearchSchema: new HotSchema({
      }),

      storeName:  'twilioPhoneNumbers',
      publicURL: '/twilioPhoneNumbers/:id',
      hotExpose: true,
      configStore: { },
    ...
    stores.twilioPhoneNumbers = new TwilioPhoneNumbers();



    // Twilio config store
    var TwilioConfig = declare( [ HotStore ], {

      schema: new HotSchema({
        number         : { type: 'number', notEmpty: true, default: '99999', trim: 50, searchable: true },
      }),

      onlineSearchSchema: new HotSchema({
      }),

      storeName:  'twilioConfig',
      publicURL: '/twilioConfig/:globalId',
      hotExpose: true,
    ...
    stores.twilioConfig = new TwilioConfig();
