  ...
  app.get( '/logs/:workspaceId', function( req, res, next ){
    ...
    stores.workspacesNinjaFormLogs.dbLayer.select( { sort: { date: 1  } }, { skipHardLimitOnQueries: true }, function( err, logLines ){
      ...
      for( var k in logLines ){
        var line = logLines[ k ];
        var tabId = line.tabId || 'server_tab'; 
        logs = logs + line.date + '\t' + line.subsystem + '\t' + tabId + '\t\[' + line.submitLabel + '\]\t' + line.line + '\n';
        if( line.data ) logs = logs + line.data;
      }
    ...
    if( err ){
      ...
      var WorkspacesInfo = declare( [ HotStore, MultiHomePermsMixin ], {
        ...
        schema: new HotSchema({
          ...
          tag         : { type: 'string', required: true, default: "Workspace's tag line",  notEmpty: true, trim: 70, searchable: true },
        }),
      ...
      var WorkspacesNinjaFormResetStatus = declare( [ JsonRestStores, MultiHomePermsMixin ], {
    
        schema: new BasicSchema({
          newStatus    : { type: 'string', required: true, default: 0 },
        }),

        storeName:  'workspacesNinjaFormResetStatus',

        DbLayer: SimpleDbLayer,
          
        publicURL: '/ninjaSubmit/ninjaFormResetStatus/:workspaceId',
        hotExpose: true,
        ...
        execAllDbFetch: function( request, cb ){
          if( Machine.workspaces[ request.params.workspaceId ] ){
            cb( null, { status: Machine.workspaces[ request.params.workspaceId ].status } );
          } else {
            cb( null, null );
          }
        },
        ...
        execPutDbUpdate: function( request, cb ){
          Machine.workspaces[ request.params.workspaceId ] = 'idle';
          cb( null, { status: 'idle' } );
        },

      });
      stores.workspacesNinjaFormResetStatus = new WorkspacesNinjaFormResetStatus();
...
var Machine = declare( null, {
  ...
  _formatMetaBody: function( meta, body ){
    var r = '';
    r = "META: " + require('util').inspect( meta, { depth: 10 } ) + '\n';
    r += "BODY:\n" + body + '\n';
    return r;
  },

  logger: function( submitLabel, line, data ){
    ...
    logInfo.workspaceId = self.workspaceId;
    logInfo.subsystem = "server";
    logInfo.tabId = "";

    logInfo.submitLabel = submitLabel;
    logInfo.line = line;    
    logInfo.data = data;

    stores.workspacesNinjaFormLogs.apiPost( logInfo, function( err ){ if( err ) console.log( err );} );
  },
  ...
  _setStatusStore: function( currentStatus ){
    ...
    //console.log( new Date() + ' ' + ms, "SETTO LO STATO A ", currentStatus );
    // This will affect the displayed status
    stores.workspacesNinjaFormStatus.apiPut( null, record, function(){} );
  },


  statusNotReady: function(){
    var self = this;

    this.status = 'notReady';
    self._setStatusStore("Form non configurata");
  },
  ...
  statusIdle: function(){
    ...
    self._setStatusStore("In idle...");
    
  },

  _statusIdleChanger: function(){
    var self = this;

    if( self.status == 'idle' ){
      var now = Math.floor( (new Date()).getTime() / 1000 );
      var unixClickDate = Math.floor( self.clickDate.getTime() / 1000 );
          
      self._setStatusStore("In idle... starting in " + ( unixClickDate - now ) + " seconds");
  ...
  statusLoadingInitial: function(){
    ...
    self.logger('', "Carico la pagina iniziale...");
  ...
  _statusLoadingInitialChanger: function( err, meta, body ){
    ...
    if( self.status == 'loadingInitial' ){
      ...
      if( err || body == '' ){
      logger( self.submitLabel, "La pagina iniziale era vuota, tentativo da rifare");
      ...
      self.logger( self.submitLabel, "La pagina iniziale e' stata scaricata", self._formatMetaBody( meta, body ) );
  ...
  statusLoadingSubsequent: function(){
    ...
    self.logger( '', "La prima pagina e' stata caricata, monitoraggio comincia ora");

    //self.config.url += "EEEEE";
  ...
  _statusLoadingSubsequentChanger: function( err, meta, body ){
    ...
    if( self.status == 'loadingSubsequent' ){
      ...
      if( err || body == '' ){
        console.log("**************************ERROR!", err );
        setTimeout( function(){
          //self._statusLoadingSubsequentChanger( null, {}, "<h1>FAKE BODY</h1>");
          fetch.fetchUrl( self.config.url, self._statusLoadingSubsequentChanger.bind( self ) );
        }, 200 );
        return;
      }
      */
      ...
      //console.log("IT IS:", err, self.subsequentLoadCounter, body.toString() );

      if( ! err && body.toString() && self.initialPageBody.toString() != body.toString() ){


        self.formPageDate = new Date( meta.responseHeaders.date );

        self.logger( '', "La prima pagina cambiata con successo e' stata generata dal server alle: " + self.formPageDate + " (tempo mandato dal SERVER); l'ULTIMA richiesta con pagina non cambiata dal server e' stata generate alle: " + self.lastUnchangedFormDate + " (tempo mandato dal SERVER);", self._formatMetaBody( meta, body )  );
        ...
        if( ! err && meta.responseHeaders && meta.responseHeaders.date ) self.lastUnchangedFormDate = new Date( meta.responseHeaders.date );
  ...
  statusSubmitting: function(){
    ...
    console.log("In statusSubmitting");
    self.status = 'submitting';
    self._setStatusStore("Submit in corso");
    self.logger( '', "Submit in corso");

    self._statusSubmittingChanger();
  },

  _statusSubmittingChanger: function(){
    var self = this;

    var $ = cheerio.load( self.formPage.toString() );
    ...
    $( 'form' ).each( function( index, form ){
      ...
      var fullAction = require('url').resolve( self.config.url, $form.attr( 'action' ) );
      ...
      //stores.workspacesNinjaFormCodes.dbLayer.select( { conditions: { and: [ { field: 'workspaceId', type: 'eq', value: self.workspaceId } ] } }, { skipHardLimitOnQueries: true }, function( err, codesData ){   });
        ...
        // It will submit each code three times
        for( var ii = 0; ii < 2; ii ++){
          ...
          codesData.forEach( function( codeData ){
            ...
            //console.log("FULL ACTION:", fullAction );
            self.logger( code, "Effettuando il submit di: " + code );
            fetch.fetchUrl( fullAction, { 
              ...
              if( err ){
                console.log( err );
                self.logger( code, "Errore durante il submit!" );
                self.logger( code, require('util').inspect( err, { depth: 10 } ) );
                // TODO: LOG ERROR
                return;
              }
  
              // TODO: LOG RESULT
              self.logger( code, "Submit fatto! Il response del server a questo submit era: " + new Date( meta.responseHeaders.date ), self._formatMetaBody( meta, body )  );
  ...
  statusAllDone: function(){
    this.status = 'allDone';
    this._setStatusStore("All done!", function(){} );
  },
...
Machine.startTicking = function(){
  ...
  setInterval( function(){
    for( var k in Machine.workspaces ){
      Machine.workspaces[ k ]._statusIdleChanger();
    }
  }, 1000 );
};
