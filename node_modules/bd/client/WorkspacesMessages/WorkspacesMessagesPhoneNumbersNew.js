...
INPUT: 
  * workspaceId
  * [contactId] - The default recipient for the message
DESCRIPTION
A form to send an SMS to the specified recipient `contactId` (`workspacesMessages`, type `sms`). If `contactId` is not specified, then it will act as a generic form to send to anybody, with "To:" effectively missing, and "Cc:" changed, visually, to "To:".
*/
...
, WorkspacesContactsTo
  ...
  return declare([ EditingWidget, ViewingWidget ], {
    ...
    postMixInProperties: function(){
      ...
      // This is for the EditingWidget part of the story
      this.storeParameters = { workspaceId: this.workspaceId };

      // This is for the ViewingWidget part of the story
      this.info = {
      ...
      if( this.contactId ){
        this.info.workspacesContacts_to = {
          storeParameters: { workspaceId: this.workspaceId },
          id: this.contactId,
          storeName: 'workspacesContacts',
        }
    ...
    renderInfo: function(){
      ...
      // Sets the default phone number as the one upon rendering
      if( this.contactId ){

        if( ! self.toWidget.get( 'value' ) ){
          var contactObject = this.info.workspacesContacts_to.data;
          self.toWidget.set( 'value', contactObject ? contactObject.mainPhoneNumberId : null );
        }
      }
