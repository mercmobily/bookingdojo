  ...
  return declare( [ EditableList ], {
    ...
    ListConstructor: declare( [ commonMixins.FullOnDemandList ], {
      ...
      renderRow: function(object, options){
        ...
        var ticked = false;
        if( object._children.workspacesContactsIsStarred[ 0 ] && object._children.workspacesContactsIsStarred[ 0 ].isStarred ) ticked = true;
        ...
        put( email, 'span.email-info', formatEmail( object._children.workspacesContactsEmails[ 0 ] ) );
        //put( email, 'span.emails-total', formatTotal( object._ref.emailsTotal ) );
        put( data, email );
        put( data, 'span.separator' );

        var phoneNumber = put( data, 'span.phone-number' );
        put( phoneNumber, 'span.phone-number-info', formatPhoneNumber( object._children.workspacesContactsPhoneNumbers[ 0 ] ) );
	...
	put( data, 'span.separator' );
        ...
        put( address, 'span.address-info', formatAddress( object._children.workspacesContactsAddresses[ 0 ] ) );
      ...
      postCreate: function(){
        ...
        self.own( 
          ...
          topic.subscribe( 'storeRecordUpdate,storeRecordCreate,storeRecordRemove', function( from, message, remote ){
           if( message.storeName == 'workspacesContactsAddresses' || message.storeName == 'workspacesContactsEmails' || message.storeName =='workspacesContactsPhoneNumbers'){
             self.refresh( { keepScrollPosition: true } );
           }
          })
    ...
    initialSort: [ { attribute: "lastName" } ],
    ...
    postCreate: function(){
      ...
      self.extraWidget.orderByNameWidget.on( 'click', function( e ){
        ...
        if( checkBox.get('value') ){
          self.listWidget.sort( [ { attribute: "firstName" } ] );
        } else {
          self.listWidget.sort( [ { attribute: "lastName" } ] );
        }
      });
