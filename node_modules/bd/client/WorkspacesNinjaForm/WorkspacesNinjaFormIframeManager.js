...
, "hotplate/bd/WorkspacesNinjaForm/workspacesNinjaFormLogger"
...
, WorkspacesNinjaFormLogBrowser
  ...
  return declare( [ widgets.DestroyableTemplatedContainer ], {
    ...
    templateString: '' +
      ...
      '        Ora sul server: <span data-dojo-attach-point="destinationServerTimeNode"></span>\n' +
      ...
      '          <div style="height: 100%;" data-dojo-props="queryOptions: { placeNew: \'first\' } , sort: [ { attribute: \'date\', descending: true} ]" data-dojo-attach-point="logsTab" data-dojo-type="hotplate/bd/WorkspacesNinjaForm/WorkspacesNinjaFormLogBrowser" title="Logs"></div>\n' +
    ...
    submitLabel: '',
    initialPage: '',
    lastUnchangedFormDate: null,
    destinationServerTime: null,
    ...
    _setDestinationServerTimeAttr: function( value ){

      // If the status is not the right one, do not update. This will prevent late requests
      // to update the header whereas the state has already changed
      if( this.status != 'loadingSubsequent' && this.status != 'loadingSubsequentPaused' )
        return;

      if( value == null ){
        this._set( 'destinationServerTime', value );
        this.destinationServerTimeNode.innerHTML = 'Not retrieved';
        return value;
      }

      if( ! this.destinationServerTime || this.destinationServerTime.getTime() < value ){ 
        this._set( 'destinationServerTime', value );
        this.destinationServerTimeNode.innerHTML = value;
        return value;
      }
    },
    ...
    _loadPage: function(){
      ...
      var localRequestTime = new Date();
      return request.get( ConfigVars.workspacesNinjaFormSettings.url, 
        {
          handleAs: 'xml',
          preventCache: true,
          timeout: 10000,
        ...
        function( res ){
          var serverResponseTime = new Date( res.getHeader( 'Date' ) );
          self.set( 'destinationServerTime', serverResponseTime );
          self._pageLoaded( res.data, localRequestTime, serverResponseTime );
        },
        function( err ){
          console.log( err );
          self._pageLoaded( '', localRequestTime );
        }
      );
    },

    _pageLoaded: function( res, localRequestTime, serverResponseTime ){
      ...
      switch( self.status ){
        ...
        case 'loadingInitial':
          ...
          if( res == '' ){
            ...
            logger( self.submitLabel, "Initial page was empty, re-attempting loading");     
            setTimeout( function(){
              self._loadPage();
            }, 150 );
...
//          console.log( res );
          ...
          if( self.initialPage != res && res != '' ){
            ...
            self._placeContentsInIframe( res );
            logger( self.submitLabel, "La prima pagina cambiata con successo e' stata generata dal server alle: " + serverResponseTime + " (tempo mandato dal SERVER); la richiesta e' stata fatta dal client alle: " + localRequestTime + " ( tempo del CLIENT); l'ULTIMA richiesta con pagina non cambiata dal server e' stata generate alle: " + self.lastUnchangedFormDate + " (tempo mandato dal SERVER);" );

          } else {

            self.lastUnchangedFormDate = self.destinationServerTime;
    ...
    _placeContentsInIframe: function( contents ){
      ...
      contents = contents.replace(/< *head *>/,'<head><base href="' + ConfigVars.workspacesNinjaFormSettings.url + '">');
    ...
    _setStatusAttr: function( value ){
      ...
      switch( status ){
        ...
        case 'closed':
          logger( self.submitLabel, "URL chiuso");
          statusString = "Chiuso";
	  self.freezeWidget.set( 'disabled', false );
	  self.reloadWidget.set( 'disabled', true );
          self.set( 'destinationServerTime', null );
          self.lastUnchangedFormDate = null;
          ...
          self.tabContainer.getChildren().forEach( function( child ){
            if( child.title != 'Main' && child.title != 'Logs' ){
        ...
        case 'loadingInitial':
          ...
          logger( self.submitLabel, "Caricando la pagina iniziale");     
	  ...
	  self.reloadWidget.set( 'disabled', true );
        ...
        case 'loadingSubsequent':
          if( self.status == 'loadingInitial' || self.status == 'loadingSubsequentPaused' ){
            logger( self.submitLabel, "Monitoraggio della form");     
        ...
        case 'loadingSubsequentPaused':
          if( self.status == 'loadingSubsequent' ){
            logger( self.submitLabel, "Monitoraggio della form in pausa");     
        ...
        case 'submitting':
          logger( self.submitLabel, "Submit della form in corso");     
	  ...
	  self.freezeWidget.set( 'disabled', true );
        ...
        case 'waitingUserInput':
          logger( self.submitLabel, "Form in display");     
      ...
      document.addEventListener('my-extension-event', function(event) {
        var message = event.detail;
          if (message.responseHeaders) {
            ...
            var parsedSettingsUrl = document.createElement('a');
            parsedSettingsUrl.href = ConfigVars.workspacesNinjaFormSettings.url;
            //parsedSettingsUrl.hostname;

            var messageUrl = document.createElement('a');
            messageUrl.href = message.url;

            if( parsedSettingsUrl.hostname == messageUrl.hostname ){

              for( var i = 0, l = message.responseHeaders.length; i < l; i ++ ){
                var item = message.responseHeaders[ i ];
                if( item.name == "Date" ){
                  var serverResponseTime = item.value;
                  logger( self.submitLabel, "Una risposta alla form e' stata generata dal server alle: " + serverResponseTime + " (tempo mandato dal SERVER);");
                }
              }
            }
          }
          ...
          if( forms ){
            ...
            forms.forEach( function( form ){
              ...
              on( iframe, 'load', function( e ){
                ...
                var now = new Date();
                logger( self.submitLabel, "Submission fatta per " + iframe.title );
