...
NOTE: It's possible to write this widget as a ViewingWidget so that contactidForSelect is no longer necessary. However, it would be an overkill, and it would require working out the contactId from the userId which would require a server-store just for that.
TODO: Make contactIdForSelect redundant, by turning this widget into a [ ViewingWidget, EditingWidget] and by using a new server store that maps a userId to a workspacesContacts record.
*/
define( [
  "dojo/_base/declare"
...
, WorkspacesContactsEmailsSelect
  ...
  return declare( [ EditingWidget ], {
    ...
    '        <div class="notificationsEmailAddress" id="${id}_notificationsEmailAddress" name="notificationsEmailAddress" data-dojo-type="hotplate/bd/WorkspacesUsersConfig/WorkspacesContactsEmailsSelect" data-dojo-props="workspaceId: this.workspaceId, contactId: this.contactIdForSelect"></div>\n' +
    '      </div>\n' +
